...
def apply_filters(class_number, filter_function, filter_function2, filter_function3):
    # Obtener imágenes
    ...
    equalizado3_ = [filter_function3(read_img(x)) for x in imagenes]
    # Agregar imágenes originales a la lista para mostrarlas
    imagenes = [cv2.imread(img_) for img_ in imagenes]
    equalizado_ = imagenes + equalizado_ + equalizado2_ + equalizado3_
...
def main():
    ...
    for class_number, class_name in enumerate(CLASS_NAMES):
        if class_number == 0:#EARLY_BLIGHT
            ...
            filter_function2 = lambda x: otsu_thresholding(gaussiano(sobel(laplaciano(convolucion_copia_borde(gaussiano(gamma_transform(x, gamma=0.4)))))))
            filter_function3 = lambda x: otsu_multi_thresholding(gaussiano(sobel(laplaciano(convolucion_copia_borde(gaussiano(gamma_transform(x, gamma=0.4)))))))
            ...
            filter_function2 = lambda x: otsu_thresholding(convolucion_copia_borde(ecualizadoRGB(gaussiano(mediana(x, ksize=9)))))
            filter_function3 = lambda x: otsu_multi_thresholding(convolucion_copia_borde(ecualizadoRGB(gaussiano(mediana(x, ksize=9)))))
        elif class_number == 2:#LEAF_MINER
            filter_function = lambda x: canny(gaussiano(gamma_transform(x), sigma=0.3))
            filter_function2 = lambda x: otsu_thresholding(gaussiano(gamma_transform(x), sigma=0.3))
            filter_function3 = lambda x: otsu_multi_thresholding(gaussiano(gamma_transform(x), sigma=0.3))
            ...
            filter_function2 = lambda x: otsu_thresholding(highboost(convolucion_copia_borde(gaussiano(gamma_transform(x, gamma=0.5), sigma=6)), k=4))
            filter_function3 = lambda x: otsu_multi_thresholding(highboost(convolucion_copia_borde(gaussiano(gamma_transform(x, gamma=0.5), sigma=6)), k=4))
        elif class_number == 4:#MOSAIC_VIRUS
            filter_function = lambda x: canny(convolucion_copia_borde(mediana(gaussiano(x))))
            filter_function2 = lambda x: otsu_thresholding(convolucion_copia_borde(mediana(gaussiano(x))))
            filter_function3 = lambda x: otsu_multi_thresholding(convolucion_copia_borde(mediana(gaussiano(x))))
            ...
            filter_function2 = lambda x: otsu_thresholding(gaussiano(convolucion_copia_borde(mediana(gaussiano(ecualizadoRGB(x))))))
            filter_function3 = lambda x: otsu_multi_thresholding(gaussiano(convolucion_copia_borde(mediana(gaussiano(ecualizadoRGB(x))))))
            ...
            filter_function2 = lambda x: otsu_thresholding(convolucion_copia_borde(mediana(gaussiano(ecualizadoRGB(gamma_transform(x)), sigma=1), ksize=7)))
            filter_function3 = lambda x: otsu_multi_thresholding(convolucion_copia_borde(mediana(gaussiano(ecualizadoRGB(gamma_transform(x)), sigma=1), ksize=7)))
            ...
            filter_function2 = lambda x: otsu_thresholding(convolucion_copia_borde(mediana(gaussiano(ecualizadoRGB(gamma_transform(x, gamma=0.33)), sigma=1), ksize=7)))
            filter_function3 = lambda x: otsu_multi_thresholding(convolucion_copia_borde(mediana(gaussiano(ecualizadoRGB(gamma_transform(x, gamma=0.33)), sigma=1), ksize=7)))
        ...
        apply_filters(class_number, filter_function, filter_function2, filter_function3)
